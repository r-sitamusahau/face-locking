...
def main():
    ...
    parser.add_argument(
        "--auto",
        action="store_true",
        help="Auto-capture samples and save when enough are collected",
    )
    parser.add_argument(
        "--camera",
        type=int,
    ...
    try:
        while True:
            ...
            if auto and total >= cfg.samples_needed:
                ...
                save_db(cfg, db, meta)
                status_msg = f"Auto-saved '{name}' to DB. Total identities: {len(db)}"
                print(status_msg)
